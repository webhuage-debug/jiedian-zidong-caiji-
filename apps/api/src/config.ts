...
const envSchema = z.object({
  ...
  SUB_CACHE_DIR: z.string().optional(),
  SUBSCRIPTION_DEFAULT_DAYS: z.coerce.number().int().positive().max(90).default(15),
  SUBSCRIPTION_DEFAULT_OUTPUT_COUNT: z.coerce.number().int().positive().max(200).default(10),
  SUBSCRIPTION_CACHE_MAX_AGE_SECONDS: z.coerce.number().int().positive().max(3600).default(300),
...
export const config = {
  ...
  SUB_CACHE_DIR: path.resolve(parsed.SUB_CACHE_DIR ?? path.join(dataDir, "sub-cache")),
