...
- 公开领取页 `/p/:slug`
- 外部用户输入正确口令后才能下载节点包
- 访问、口令输入、口令正确/错误、解锁、下载、反馈统计
- 来源平台参数记录，例如 `?from=youtube_desc`
- 简单反馈入口：地区、运营商、设备、使用软件、是否可用、备注
- 公开视频模式设置保存到后端
- 公开视频模式下隐藏来源 URL、失败原因中的敏感片段、管理员账号
- 日志输出接口支持敏感信息脱敏
- 公开下载接口增加按 IP 哈希和批次的分钟级限速
- 后台提供批次统计接口和反馈数据接口

## 规划功能

- 后续版本：更完整的协议级可用性测试、多来源质量评分、批次状态管理增强、自动化测试覆盖。
