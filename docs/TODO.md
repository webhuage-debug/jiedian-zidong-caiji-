...
- 在 VPS Docker 环境构建并验证 v1.1.0：后台登录、订阅管理页面、创建订阅活动、领取页口令解锁、raw/base64 订阅访问。
- 验证订阅缓存目录 `SUB_CACHE_DIR` 不会进入 Git，缓存文件只保留在 VPS 数据卷。
- 验证 `/sub/:token/raw` 与 `/sub/:token/base64` 访问不会触发 Xray 检测、采集任务或复杂实时筛选。
- VPS 实测订阅自动维护定时任务：确认应用启动后默认每 300 秒自动执行到期订阅活动健康检查，并更新最近检查时间、最近替换数量和缓存刷新时间。
- 后续完善地区识别：当前地区分组基于来源和节点内容关键词，后续可接入更可靠的 GeoIP / 节点名称解析。
- 后续完善 Clash YAML、sing-box JSON 订阅格式；当前主流程只提供 raw/base64 通用订阅。
