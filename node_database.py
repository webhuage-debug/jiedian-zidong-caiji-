...
class NodeDatabase:
    ...
    def cleanup_stale_unvalidated_nodes(self, days: int = 3) -> Dict[str, int]:
    ...
    def refresh_premium_subscription_pool(
        ...
        def add(row: Dict[str, object], reason: str) -> None:
            ...
            row["premium_score"] = float(row.get("quality_score") or 0)

        primary = [row for row in rows if (not prefer_asia or row.get("asia"))]
        ...
        for bucket, reason, strict in (
            ...
            (fallback, "global_low_latency", True),
            (primary, "asia_relaxed" if prefer_asia else "relaxed", False),
            (fallback, "global_relaxed", False),
        ):
            for row in bucket:
