...
def proxy_uri(proxy: dict) -> Optional[str]:
    ...
    for source, target in (
        ("network", "type"), ("flow", "flow"), ("servername", "sni"), ("sni", "sni"),
        ("security", "security"), ("alpn", "alpn"), ("client-fingerprint", "fp"),
        ("client_fingerprint", "fp"), ("skip-cert-verify", "allowInsecure"),
    ):
        if proxy.get(source) not in (None, "", False):
            query[target] = proxy[source]
